...
def _inplace_to_torch_windows(udctwin: UDCTWindows) -> None:
    for ires in range(len(udctwin)):
        for idim in range(len(udctwin[ires])):
            for iang in range(len(udctwin[ires][idim])):
                udctwin[ires][idim][iang] = [
                    torch.as_tensor(arr) for arr in udctwin[ires][idim][iang]
                ]


def _inplace_to_torch_decimation(decimation_ratio: list) -> None:
    for ires in range(len(decimation_ratio)):
        decimation_ratio[ires] = torch.as_tensor(decimation_ratio[ires])


def udctmdwin(
    param_udct: ParamUDCT,
) -> tuple[UDCTWindows, list[torch.Tensor], dict[int, dict[int, np.ndarray]]]:
    ...
    _inplace_to_torch_windows(udctwin)
    _inplace_to_torch_decimation(decimation_ratio)
