...
def fun_meyer(x: npt.NDArray, param: list[float]) -> npt.NDArray:
    """
...
def bands2vec(imband: dict[tuple[int, ...], npt.NDArray]) -> npt.NDArray:
    """
...
def vec2bands(imband: npt.NDArray, udct: udct) -> dict[tuple[int, ...], npt.NDArray]:
    """
    Convert vector banck to a dictionary of subband
    """
    imSz = np.array(udct.sz) // 2 ** (udct.res - 1)
    # first is the low band
    uncompressed: dict[tuple[int, ...], npt.NDArray] = {
        (0,): np.reshape(imband[: np.prod(imSz)], imSz)
    }
    p = np.prod(imSz)
    for id in udct.Msubwin:
        # if id == 0: continue
...
def ucurv2d_show(imband: dict[tuple[int, ...], npt.NDArray], udct: udct) -> npt.NDArray:
    if udct.dim != 2:
        err = "ucurv2d_show only works with 2D transform"
        raise Exception(err)
    cfg = udct.cfg
    imlist = []
    res = udct.res
    ...
    return np.concatenate([iml, dimg2], axis=1)
