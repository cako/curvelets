"""
Curvelet vs Wavelet at Highest Scale
====================================
...
between directional curvelet windows and isotropic wavelet windows, showing
both frequency-domain windows and spatial coefficients.
"""
...
print(f"Wavelet scale 1 bands: {len(coeffs_wavelet[1])}")  # noqa: T201
...
# Curvelet windows are directional (wedges), while wavelet windows are
# separable (product of 1D filters).
...
wavelet_window_0_2d = np.outer(lowpass_1d, highpass_1d)  # Low-High
wavelet_window_1_2d = np.outer(highpass_1d, lowpass_1d)  # High-Low
wavelet_window_2_2d = np.outer(highpass_1d, highpass_1d)  # High-High

# Apply fftshift to center the frequency domain for visualization
wavelet_windows_scale1 = [
    fftshift(wavelet_window_0_2d),
    fftshift(wavelet_window_1_2d),
    fftshift(wavelet_window_2_2d),
]
...
# Plot curvelet windows
n_curvelet_windows = len(curvelet_windows_scale1)
n_cols = max(n_curvelet_windows, 3)  # At least 3 columns to match wavelet
...
# Extract wavelet coefficients for scale 1 (highpass bands)
wavelet_coeffs_scale1 = coeffs_wavelet[1]
...
n_cols = max(n_curvelet_coeffs, 3)  # At least 3 columns to match wavelet
fig, axs = plt.subplots(2, n_cols, figsize=(4 * n_cols, 8))
...
# Bottom row: Wavelet coefficients
wavelet_coeff_names = [
    "Wavelet Highpass 0\n(Low-High)",
    "Wavelet Highpass 1\n(High-Low)",
    "Wavelet Highpass 2\n(High-High)",
...
for i, (coeff, name) in enumerate(zip(wavelet_coeffs_scale1, wavelet_coeff_names)):
    ax = axs[1, i]
    # Wavelet coefficients are real-valued
