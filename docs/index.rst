...
.. [#monogenic-ftn] The monogenic curvelet transform is only defined for 2D signals according to Storath 2010. While the implementation accepts arbitrary dimensions, only the first two Riesz components (R_1 and R_2) are used, which is mathematically correct only for 2D inputs.
